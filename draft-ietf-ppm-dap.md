...
Output share:
: An aggregator's share of the output of the VDAF
   {{!VDAF=I-D.draft-irtf-cfrg-vdaf-03}} preparation step. Many output shares are
...
{{!VDAF=I-D.draft-irtf-cfrg-vdaf-03}} in order to prepare a report for aggregation. At
...
particularly important for controlling the amount of noise added to reports
...
This message is called the Client's report. It consists of a "header", the
"public share" output by the VDAF's input-distribution algorithm, and the
...
`agg_param` is the opaque aggregation parameter distributed to the aggregtors by
